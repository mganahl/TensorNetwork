...
def test_matvec():
  ...
  def matvec(mps_tensor):
      return stoq.mat_vec(L,R,Lgate, Rgate,mpo_tensor=mpo_blocked[site], backend='tensorflow', 
                          site=site, mps_tensor=mps_tensor)
  ...
  stoq.position(mps, site)
  e, opt=stoq._optimize_1s_local(mps,site,sweep_dir='r',ncv=100,precision=1E-13,delta=1E-10)
...
def test_J1J2_mpo(N1=4, N2=2, J1=1, J2=1):
  ...
  ncv=30
  precision=1E-12
  mps = MPS.FiniteMPSCentralGauge.random(d=[2]*N1*N2, D=[D]*(N1*N2-1),dtype=dtype)
  mpo = MPO.Finite2D_J1J2(J1=J1, J2=J2, N1=N1, N2=N2, dtype=dtype)
  dmrg = DMRG.FiniteDMRGEngine(mps,mpo)
  e_dmrg=dmrg.run_one_site(verbose=1,Nsweeps=20,ncv=ncv,precision=precision)
