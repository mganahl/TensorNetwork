...
def do_lanczos_compiled(L, mpo, R, initial_state, ncv, delta, reortho=False):
  ...
  def do_lanczos_step(n, lanstate):
    ...
    #alpha=misc_mps.ncon([tf.math.conj(xn),Hxn],[[1,2,3],[1,2,3]])
    # alpha = misc_mps.ncon([
    #     tf.reshape(tf.math.conj(xn), [xn.shape[0] * xn.shape[1] * xn.shape[2]]),
...
def tridiag_tensorflow(vecs, alpha, beta):
  Heff = np.diag(alpha) + np.diag(beta, -1) + np.diag(tf.math.conj(beta), 1)
...
class LanczosEngine:
  ...
  def _simulate(self, initialstate, reortho=False, verbose=False):
    ...
    while converged == False:
      ...
      Hxn = self.matvec(xn)
      epsn.append(
          misc_mps.ncon([tf.math.conj(xn), Hxn],
                        [range(len(xn.shape)),
                         range(len(Hxn.shape))]))
    ...
    for n2 in range(min(1, eta.shape[0])):
      ...
      states.append(state / tf.sqrt(
          misc_mps.ncon([tf.math.conj(state), state],
                        [range(len(state.shape)),
                         range(len(state.shape))])))
    return eta[0], states[0], converged


def compile_lanczos(on=False, simple=False):
