...
def get_generator_from_gate(g):
    ...
    if g.dtype in (tf.complex128, tf.complex64):
        ...
        matrix = tf.linalg.logm(
            tf.complex(tf.reshape(g, (dout, din)),
    ...
    for s in range(0, len(gates), 2):
        if gates[(s, s + 1)].dtype in (tf.complex128, tf.complex64):
            gates[(s, s + 1)] += tf.complex(
                tf.random_uniform(shape=gates[(s, s + 1)].shape,
...
def randomize_gates(gates, noise):
    ...
    for k in gates.keys():
        if gates[k].dtype in (tf.complex128, tf.complex64):
            gates[k] += tf.complex(
                tf.random_uniform(shape=gates[k].shape,
            ...
            gates[k] += tf.random_uniform(shape=gates[k].shape,
                                          dtype=gates[k].dtype.real_dtype,
                                          minval=-noise / 2,
                                          maxval=noise / 2)
    return gates
...
def initialize_even_two_body_generators(ds, dtype, which='identities'):
    ...
    if which in ('i', 'identities', 'eye', 'e'):
        for site in range(0, len(ds) - 1, 2):
            if dtype in (tf.float32, tf.float64):
                two_body_generators[(site, site + 1)] = tf.eye(ds[site] *
                                                               ds[site + 1],
                                                               dtype=dtype)
            elif dtype in (tf.complex128, tf.complex64):
        ...
        for site in range(0, len(ds) - 1, 2):
            if dtype in (tf.float32, tf.float64):
                ...
                two_body_generators[(site, site + 1)] = tf.complex(
                    tf.random_uniform(shape=(ds[site] * ds[site + 1],
                                             ds[site] * ds[site + 1]),
                                      dtype=dtype.real_dtype,
                                      minval=-0.1,
                                      maxval=0.1),
...
def initialize_one_body_generators(ds, dtype, which='identities'):
    ...
    if which in ('i', 'identities', 'eye', 'e'):
        if dtype in (tf.float32, tf.float64):
            one_body_generators = {
                site: tf.eye(ds[site], dtype=dtype)
                for site in range(0, len(ds))
            }
        elif dtype in (tf.float64, tf.complex128):
            one_body_generators = {
                site: tf.complex(tf.eye(ds[site], dtype=dtype.real_dtype),
        ...
        if dtype in (tf.float32, tf.float64):
            one_body_generators = {
                site: tf.random_uniform(shape=(ds[site], ds[site]),
                                        dtype=dtype.real_dtype,
                                        minval=-0.1,
                                        maxval=0.1)
                for site in range(0, len(ds))
            ...
            one_body_generators = {
                site: tf.complex(
                    tf.random_uniform(shape=(ds[site], ds[site]),
                                      dtype=dtype.real_dtype,
                                      minval=-0.1,
                                      maxval=0.1),
                    tf.random_uniform(shape=(ds[site], ds[site]),
...
class OverlapMaximizer:
    ...
    def __init__(self,
                 ...
                 one_body_gates=None,
                 two_body_gates=None,
                 name='overlap_minimizer'):
        """
        initialize an OverlapMaximizer object
        This object minimizes the overlap between `mps` and a reference mps 
    ...
    def reset_two_body_gates(self, which='identities', dtype=None, noise=0.0):
        ...
        self.two_body_gates = initialize_even_two_body_gates(self.mps.d,
                                                             self.mps.dtype,
                                                             which,
                                                             noise=noise)
        self.two_body_gates.update(
            initialize_odd_two_body_gates(self.mps.d,
                                          self.mps.dtype,
                                          which,
                                          noise=noise))

    def reset_even_two_body_gates(self,
                                  which='identities',
                                  dtype=None,
                                  noise=0.0):
        """
        ...
        """
        self.two_body_gates.update(
            initialize_even_two_body_gates(self.mps.d,
                                           self.mps.dtype,
                                           which,
                                           noise=noise))

    def reset_odd_two_body_gates(self,
                                 which='identities',
                                 dtype=None,
                                 noise=0.0):
        """
    ...
    def reset_one_body_gates(self, which='eye', dtype=None, noise=0.0):
        ...
        if which in ('e', 'eye', 'h', 'haar', 'i', 'identities'):
            self.one_body_gates = initialize_one_body_gates(self.mps.d,
                                                            self.mps.dtype,
                                                            which,
                                                            noise=noise)
        else:
            ...
            for site in range(len(ds)):
                self.one_body_gates[site] = get_gate_from_generator(
                    gen[site], (ds[site], ds[site]))

    @staticmethod
    def add_unitary_batched_right(site, right_envs, mps, samples,
                                  one_body_gates, two_body_gates):
        """ 
        ...
        if site == (len(mps) - 1):
            ...
            tmp = tf.reshape(tmp,
                             (Nt, D * din * dout, d))  #(Nt, D * din * dout, d)
            tmp2 = tf.expand_dims(
                tf.one_hot(samples[:, site], mps.d[site], dtype=mps.dtype),
                2)  #(Nt, d, 1)
            right_envs[site - 1] = tf.reshape(tf.matmul(tmp, tmp2),
                                              (Nt, D, din, dout))
    ...
    def add_unitary_batched_left(site, left_envs, mps, samples, one_body_gates,
        ...
        if site == 0:
            ...
            left_envs[site + 1] = tf.reshape(tf.matmul(tmp, tmp2),
                                             (Nt, D, di, do))
    ...
    def get_two_body_env_batched(sites, left_envs, right_envs, one_body_gates,
        ...
        if (sites[0] > 0) and (sites[1] < len(mps) - 1) and (sites[0] % 2 == 0):
            ...
            bnet.batched_contract_between(lenv_node, left_sample_node,
                                          lenv_node[0], left_sample_node[0])
            bnet.batched_contract_between(renv_node, right_sample_node,
                                          renv_node[0], right_sample_node[0])
            bnet.contract(e6)
            bnet.contract(e7)
        ...
        #odd sites
        elif (sites[0] > 0) and (sites[1] < len(mps) - 1) and (sites[0] %
                                                               2 == 1):
            ds = mps.d
            bnet = btn.BatchTensorNetwork()
            ...
            left_sample_node = bnet.add_node(
                tf.expand_dims(
                    tf.one_hot(samples[:, sites[0]],
                               ds[sites[0]],
                               dtype=mps.dtype), 2))
            right_sample_node = bnet.add_node(
                tf.expand_dims(
                    tf.one_hot(samples[:, sites[1]],
                               ds[sites[0]],
                               dtype=mps.dtype), 2))
            right_tensor_node = bnet.add_node(mps.get_tensor(sites[1]))
            right_unitary_node = bnet.add_node(one_body_gates[sites[1]])
            ...
            order = [
                lenv_node[0], left_sample_node[1], right_sample_node[1],
                lenv_node[3], renv_node[3]
            ]
            ...
            ctmp = bnet.batched_contract_between(ltmp, rtmp, lenv_node[0],
                                                 renv_node[0])
            ctmp = bnet.batched_contract_between(ctmp, left_sample_node,
                                                 lenv_node[0],
                                                 left_sample_node[0])
            out = bnet.batched_contract_between(ctmp, right_sample_node,
                                                lenv_node[0],
                                                right_sample_node[0])
            out = out.reorder_edges(order)
            return out.tensor
            ...
            order = [
                renv_node[0], left_sample_node[1], renv_node[2],
            ...
            out = bnet.batched_contract_between(tmp, left_sample_node,
                                                renv_node[0],
                                                left_sample_node[0])
            out = out.reorder_edges(order)
            return out.tensor
            ...
            bnet.contract(e3)
            tmp = bnet.contract(e2)
            out = bnet.batched_contract_between(tmp, right_sample_node,
                                                lenv_node[0],
                                                right_sample_node[0])
            out = out.reorder_edges(order)
            return out.tensor
    ...
    def get_one_body_env_batched(site, left_envs, right_envs, mps, samples):
        if (site not in (0, len(mps) - 1)) and (site % 2 == 1):
            ...
            sample_node = bnet.add_node(
                tf.one_hot(samples[:, site], ds[site], dtype=mps.dtype))
            tensor_node = bnet.add_node(
                tf.squeeze(mps.get_tensor(site),
                           2))  #remove trailing dimension from mps tensor
    ...
    def overlap_batched(site, left_envs, right_envs, one_body_gates, mps,
        ...
        env = tf.math.reduce_mean(OverlapMaximizer.get_one_body_env_batched(
            site, left_envs, right_envs, mps, samples),
                                  axis=0)
    ...
    def compute_left_envs_batched(self, samples):
        [
            self.add_unitary_batched_left(site, self.left_envs_batched,
                                          self.mps, samples,
                                          self.one_body_gates,
                                          self.two_body_gates)
            for site in range(len(self.mps) - 1)
    ...
    def one_body_gradient(site, left_envs, right_envs, mps, conj_mps,
        ...
        """
        env = OverlapMaximizer.get_one_body_env(site, left_envs, right_envs,
                                                mps, conj_mps)
    ...
    def two_body_gradient(sites, left_envs, right_envs, one_body_gates, mps,
        ...
        with tf.GradientTape() as tape:
            ...
            if mps.dtype in (tf.complex128, tf.complex64):
                tmp = misc_mps.ncon([
                    tf.reshape(env, (ds[sites[0]] * ds[sites[1]],
    ...
    def one_body_gradient_cost_function_batched(site,
        ...
        """
        env = OverlapMaximizer.get_one_body_env_batched(site, left_envs,
                                                        right_envs, mps,
                                                        samples)
        ds = mps.d
                ...
                if activation is not None:
                    C = tf.complex(
                        tf.math.reduce_mean(
                            ...
                            axis=0),
                        tf.zeros(shape=[1], dtype=mps.dtype.real_dtype))
                else:
                    C = tf.complex(
                        tf.math.reduce_mean((1 - gamma) * tf.real(psi_sigma) -
                                            gamma * np.abs(tf.imag(psi_sigma)),
                                            axis=0),
                        tf.zeros(shape=[1], dtype=mps.dtype.real_dtype))
        ...
        if mps.dtype in (tf.complex128, tf.complex64):
            g2 = tf.complex(tf.real(tape.gradient(log_psi, g)),
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            if mps.dtype in (tf.complex128, tf.complex64):
                ...
                C = tf.math.reduce_mean(
                    psi_sigma, axis=0) + tf.math.reduce_mean(tf.conj(psi_sigma),
                                                             axis=0)
            elif mps.dtype in (tf.float64, tf.float32):
                psi_sigma = misc_mps.ncon(
                    [env, tf.linalg.expm(g - tf.conj(tf.transpose(g)))],
    ...
    def test_one_body_gradient_overlap_batched(site, left_envs, right_envs, mps,
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            if mps.dtype in (tf.complex128, tf.complex64):
                ...
                C1 = (tf.math.reduce_sum(psi_sigma, axis=0) +
                      tf.math.reduce_sum(tf.conj(psi_sigma), axis=0)) / np.sqrt(
                          samples.shape[0])
                C2 = tf.math.reduce_sum(psi_sigma * tf.conj(psi_sigma),
                                        axis=0)  #the norm
            elif mps.dtype in (tf.float64, tf.float32):
                psi_sigma = misc_mps.ncon(
                    [env, tf.linalg.expm(g - tf.conj(tf.transpose(g)))],
                    [[-1, 1, 2], [1, 2]])
                avsigns = np.average(np.sign(psi_sigma.numpy()))
                C1 = tf.math.reduce_sum(psi_sigma, axis=0) / np.sqrt(
                    samples.shape[0])
    ...
    def two_body_gradient_batched(sites,
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            if mps.dtype in (tf.complex128, tf.complex64):
                psi_sigma = misc_mps.ncon([
                    tf.reshape(env,
                               (samples.shape[0], ds[sites[0]] * ds[sites[1]],
                                ds[sites[0]] * ds[sites[1]])),
                    tf.linalg.expm(g - tf.conj(tf.transpose(g)))
                ], [[-1, 1, 2], [1, 2]])
                avsigns = np.average(np.sign(np.real(psi_sigma.numpy(
    ...
    def absorb_one_body_gates(self):
        ...
        tensors = [
            misc_mps.ncon(
                [self.mps.get_tensor(site), self.one_body_gates[site]],
    ...
    def minimize_layerwise(self, ref_mps, num_sweeps, alpha=1.0, verbose=0):
        ...
        for it in range(num_sweeps):
            for site in range(len(self.mps) - 1):
                env = self.two_body_gates[(site, site + 1)] * (
                    1 - alpha) + alpha * self.get_two_body_env(
                        (site, site + 1), selff.left_envs, self.right_envs,
                        one_body_gates, self.mps, ref_mps)
                self.two_body_gates[(site, site +
                                     1)] = self.two_body_update_svd_numpy(env)
                self.add_unitary_left(site, self.left_envs, self.mps, ref_mps,
                                      self.one_body_gates, self.two_body_gates)
                if verbose > 0 and site > 0:
                    overlap = self.overlap(site, self.left_envs,
                                           self.right_envs, self.one_body_gates,
                                           self.mps, ref_mps)
            ...
            for site in range(len(self.mps) - 1):
                if site in sites:
                    ...
                    if samples != None:
                        env += (
                            alpha_samples *
                            tf.math.reduce_mean(self.get_one_body_env_batched(
                                site, self.left_envs_batched,
                                self.right_envs_batched, self.mps, samples),
                                                axis=0))
                    if ref_mps != None:
                        env += (alpha_ref_mps * self.get_one_body_env(
                            site, self.left_envs, self.right_envs, self.mps,
                            ref_mps))
                    self.one_body_gates[site] = self.one_body_update_svd_numpy(
                        env)
                ...
                if site < (len(self.mps) - 1):
                    if samples != None:
                        self.add_unitary_batched_left(site,
                                                      self.left_envs_batched,
                                                      self.mps, samples,
                                                      self.one_body_gates,
                                                      self.two_body_gates)
                    if ref_mps != None:
                        self.add_unitary_left(site,
                                              self.left_envs,
                                              self.mps,
                                              ref_mps,
                                              self.one_body_gates,
    ...
    def minimize_even(self,
        ...
        self.minimize_two_body(samples=samples,
                               ref_mps=ref_mps,
    ...
    def gradient_minimization_two_body(self,
        ...
        for it in range(num_sweeps):
            ...
            if ref_mps != None:
                [
                    self.add_unitary_right(site, self.right_envs, self.mps,
                                           ref_mps, self.one_body_gates,
                                           self.two_body_gates)
                    for site in reversed(range(1, len(self.mps)))
                ]
            ...
            for site in range(len(self.mps) - 1):
                ...
                if site in sites:
                    ...
                    if opt_type in ('sequential', 'seq'):
                        ...
                        self.two_body_gates[(site, site +
                                             1)] = get_gate_from_generator(
                                                 two_body_generators[(site,
                                                                      site +
                                                                      1)],
                                                 (ds[site], ds[site + 1],
                                                  ds[site], ds[site + 1]))
                        ...
                        raise ValueError(
                            'unknown value {} for opt_type'.format(opt_type))
                if site < (len(self.mps) - 1):
                    if samples != None:
                        self.add_unitary_batched_left(site,
                                                      self.left_envs_batched,
                                                      ...
                                                      self.two_body_gates)
                    if ref_mps != None:
                        self.add_unitary_left(site, self.left_envs, self.mps,
                                              ref_mps, self.one_body_gates,
                                              self.two_body_gates)
                if (verbose > 0) and (site not in (0, len(self.mps) - 1)):
                    ...
                    if (ref_mps != None) and (samples != None):
                        stdout.write(
                            ...
                            % (it, num_sweeps, site, np.rea(c1), np.imag(c1),
                               np.real(avsign), np.imag(avsign), np.real(c2),
                               np.imag(c2)))
            ...
            if opt_type in ('sequentual', 'seq'):
                ...
                for site in sites:
                    ...
                    self.two_body_gates[(site,
                                         site + 1)] = get_gate_from_generator(
                                             two_body_generators[(site,
                                                                  site + 1)],
                                             (ds[site], ds[site + 1], ds[site],
                                              ds[site + 1]))
            else:
                raise ValueError(
                    'unknown value {} for opt_type'.format(opt_type))
    ...
    def gradient_minimization_overlap_one_body(self,
        ...
        for it in range(num_sweeps):
            ...
            for site in range(len(self.mps)):
                ...
                if site in sites:
                    grad, overlap = self.one_body_gradient(
                        site, self.left_envs, self.right_envs, self.mps,
                ...
                if (verbose > 0) and (site not in (0, len(self.mps) - 1)):
                    stdout.write(
                        ...
                        % (it, num_sweeps, site, np.real(overlap),
                           np.imag(overlap)))
                    stdout.flush()
                if verbose > 1:
                    print()
    ...
    def gradient_minimization_cost_function_overlap_one_body_batched(
        ...
        for it in range(num_sweeps):
            ...
            for site in range(len(self.mps)):
                ...
                if site in sites:
                    grad = tf.zeros(shape=[ds[site], ds[site]],
                                    dtype=self.mps.dtype)
                    if samples != None:
...
class MPOStoquastisizer:
    def __init__(self,
                 mpo,
                 one_body_gates=None,
                 two_body_gates=None,
                 name='MPOstoquastisizer'):
        """
